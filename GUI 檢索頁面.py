...
class ExchangeStd (tk.Frame):
    ...
    def callback(self, *args):
        ...
        if test == "TOEFL iBT":
            ...
            self.toefl = tk.OptionMenu(self, self.variables, *tof)
            self.toefl.config(width=10, font=('Helvetica', 12))
            self.toefl.grid(row=8, column=3)
        if test == "IELTS":
            self.variables = tk.StringVar(self)
            self.toefl = tk.OptionMenu(self, self.variables, *iel)
            self.toefl.config(width=10, font=('Helvetica', 12))
            self.toefl.grid(row=8, column=3)
        if test == "全民英檢":
            self.variables = tk.StringVar(self)
            self.toefl = tk.OptionMenu(self, self.variables, *gept)
            self.toefl.config(width=10, font=('Helvetica', 12))
            self.toefl.grid(row=8, column=3)
        if test == "法語檢定":
            self.variables = tk.StringVar(self)
            self.toefl = tk.OptionMenu(self, self.variables, *fra)
            self.toefl.config(width=10, font=('Helvetica', 12))
            self.toefl.grid(row=8, column=3)
        if test == "德語檢定":
            self.variables = tk.StringVar(self)
            self.toefl = tk.OptionMenu(self, self.variables, *deu)
            self.toefl.config(width=10, font=('Helvetica', 12))
            self.toefl.grid(row=8, column=3)
    def select(self, *args):
        ...
        if country == 1:
            ...
            self.toefl = tk.OptionMenu(self, self.var, *asia)
            self.toefl.config(width=10, font=('Helvetica', 12))
            self.toefl.grid(row=3, column=3)
        if country == 2:
            self.var = tk.StringVar(self)
            self.toefl = tk.OptionMenu(self, self.var, *europe)
            self.toefl.config(width=10, font=('Helvetica', 12))
            self.toefl.grid(row=3, column=3)
        if country == 3:
            self.var = tk.StringVar(self)
            self.toefl = tk.OptionMenu(self, self.var, *america_N)
            self.toefl.config(width=10, font=('Helvetica', 12))
            self.toefl.grid(row=3, column=3)
        if country == 4:
            self.var = tk.StringVar(self)
            self.toefl = tk.OptionMenu(self, self.var, *america_S)
            self.toefl.config(width=10, font=('Helvetica', 12))
            self.toefl.grid(row=3, column=3)
        if country == 5:
            self.var = tk.StringVar(self)
            self.toefl = tk.OptionMenu(self, self.var, *african)
            self.toefl.config(width=10, font=('Helvetica', 12))
            self.toefl.grid(row=3, column=3)
        if country == 6:
            self.var = tk.StringVar(self)
            self.toefl = tk.OptionMenu(self, self.var, *ocean)
            self.toefl.config(width=10, font=('Helvetica', 12))
            self.toefl.grid(row=3, column=3)
    def createWidgets(self):
        college = ["文", "理", "社科", "醫", "工", "生農", "管理", "公衛", "電資", "法", "生命科學"]
        language = ["TOEFL iBT", "IELTS", "全民英檢", "日文檢定", "韓語檢定", "法語檢定", "德語檢定"]
        imgname = Image.open("Desktop/123.png")
        ...
        self.label2 = tk.Label(self, text="洲別：", bg="pink", font=f1)
        self.label4 = tk.Label(self, text="語言檢定：", bg="pink", font=f1)
        self.label5 = tk.Label(self, text="GPA：", bg="pink", font=f1)
        self.label6 = tk.Label(self, text="你是什麼學院：", bg="pink", font=f1)
        self.button = tk.Button(self, text = "Search", bg="yellow", command=self.clickBtn, font=f1)
        self.txtNum_GPA = tk.Text(self, height=1, bg="white", font=f1)
        self.var1 = tk.IntVar(self)
        self.checkNum1 = tk.Checkbutton(self, text="亞洲", variable=self.var1, offvalue=0, onvalue=1, height=1, width=10, bg="pink", command=self.select, font=f1)
        self.checkNum2 = tk.Checkbutton(self, text="歐洲", variable=self.var1, offvalue=0, onvalue=2, height=1, width=10, bg="pink", command=self.select, font=f1)
        self.checkNum3 = tk.Checkbutton(self, text="北美洲", variable=self.var1, offvalue=0, onvalue=3, height=1, width=10, bg="pink", command=self.select, font=f1)
        self.checkNum4 = tk.Checkbutton(self, text="中南美洲", variable=self.var1, offvalue=0, onvalue=4, height=1, width=10, bg="pink", command=self.select, font=f1)
        self.checkNum5 = tk.Checkbutton(self, text="非洲", variable=self.var1, offvalue=0, onvalue=5, height=1, width=10, bg="pink", command=self.select, font=f1)
        self.checkNum6 = tk.Checkbutton(self, text="大洋洲", variable=self.var1, offvalue=0, onvalue=6, height=1, width=10, bg="pink", command=self.select, font=f1)
    ...
    def clickBtn(self):
        self.variable1.get()  # 語言檢定
        self.var1.get()  # 洲別
        self.var.get()  # 國家
        self.variables.get()  # 檢定級數
